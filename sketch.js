...
function setup() 
{
  ...
  star_bar = createSprite(50,20,30,30);
  star_bar.scale = 0.2;
  star_bar.addAnimation('empty',empty_star);
  star_bar.addAnimation('one',one_star);
  star_bar.addAnimation('two',two_star);
  star_bar.changeAnimation('empty');
  ...
  blover = createImg('baloon2.png');
  blover.position(260,370);
  blover.size(120,120);
  blover.mouseClicked(airblow);
...
function draw() 
{
  ...
  if(fruit!=null && fruit.position.y>=650)
  {
   ...
   {
     star.visible = false;
     star_bar.changeAnimation('one');
   }

   if(collide(fruit,star2,40)==true)
   {
     star2.visible= false;
     star_bar.changeAnimation('two');
   }   
